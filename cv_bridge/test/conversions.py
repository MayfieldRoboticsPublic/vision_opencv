...
class TestConversions(unittest.TestCase):
    ...
    def test_mono16_cv2(self):
        ...
        self.assertRaises(CvBridgeError, lambda: br.imgmsg_to_cv2(
            br.cv2_to_imgmsg(im), "mono16")
        )
    ...
    def test_encode_decode_cv2(self):
        import cv2
        import numpy as np
        fmts = [cv2.CV_8U, cv2.CV_8S, cv2.CV_16U, cv2.CV_16S,
                cv2.CV_32S, cv2.CV_32F, cv2.CV_64F]
            ...
            for h in range(100, 800, 100):
                for f in fmts:
                    for channels in ([], 1, 2, 3, 4, 5):
                        if channels == []:
                            original = np.uint8(
                                np.random.randint(0, 255, size=(h, w))
                            )
                        else:
                            original = np.uint8(
                                np.random.randint(0,
                        ...
                        if channels == 1:
                            ...
                            self.assert_(
                                original.shape[:2] == newimg.shape[:2]
                            )
                        else:
    ...
    def test_encode_decode_cv2_compressed(self):
        ...
        formats = ["jpg", "jpeg", "jpe", "png", "bmp", "dib", "ppm",
                   "pgm", "pbm", "jp2", "sr", "ras", "tif", "tiff"]
            ...
            for h in range(100, 800, 100):
                for f in formats:
                    for channels in ([], 1, 3):
                        ...
                        compress_rosmsg = cvb_en.cv2_to_compressed_imgmsg(
                            original, f
                        )
                        newimg = cvb_de.compressed_imgmsg_to_cv2(
                            compress_rosmsg
                        )
                        self.assert_(original.dtype == newimg.dtype)
                        if channels == 1:
    ...
    def test_endianness(self):
        ...
        self.assert_(msg.is_bigendian is True)
